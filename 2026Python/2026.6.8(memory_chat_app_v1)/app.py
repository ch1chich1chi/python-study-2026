...
def save_session():
        if st.session_state.current_session:
            ...
            # 保存会话数据
            with open(get_session_path(st.session_state.current_session), "w", encoding="utf-8") as f:
...
def load_session(session_name):
    try:
        if os.path.exists(get_session_path(session_name)):
        # 读取会话数据
            with open(get_session_path(session_name),"r",encoding="utf-8") as f:
...
def delete_session(session_name):
    try:
        if os.path.exists(get_session_path(session_name)):
            os.remove(get_session_path(session_name)) # 删除文件
...
if prompt: # 字符串会自动转化为布尔值,如果字符串非空,则为True;否则为False
    ...
    response = stream_chat_response(messages_for_llm)
